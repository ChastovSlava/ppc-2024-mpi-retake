...
TEST(chastov_v_algorithm_cannon_seq, test_empty) {
  ...
  task_data_seq->outputs.emplace_back(reinterpret_cast<uint8_t *>(resultMatrix.data()));
...
TEST(chastov_v_algorithm_cannon_seq, test_multiplication_1x1) {
  ...
  std::vector<double> expectedResult = Multiplication(matrix1, matrix2, kMatrix);
  ...
  chastov_v_algorithm_cannon_seq::TestTaskSequential testTaskSequential(task_data_seq);
  ...
  ASSERT_TRUE(std::equal(expectedResult.begin(), expectedResult.end(), resultMatrix.begin(),
...
TEST(chastov_v_algorithm_cannon_seq, test_zero_matrix) {
  ...
  testTaskSequential.RunImpl();
  testTaskSequential.PostProcessingImpl();
  ASSERT_TRUE(std::equal(expected.begin(), expected.end(), resultMatrix.begin(),
                         [](double a, double b) { return std::abs(a - b) < 1e-9; }));
}
...
TEST(chastov_v_algorithm_cannon_seq, test_reverse_matrix) {
  ...
  testTaskSequential.RunImpl();
  testTaskSequential.PostProcessingImpl();
  ASSERT_TRUE(std::equal(iden.begin(), iden.end(), resultMatrix.begin(),
                         [](double a, double b) { return std::abs(a - b) < 1e-9; }));
}
