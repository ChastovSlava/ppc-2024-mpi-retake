...
#include <boost/mpi/operations.hpp>  // Для isend, irecv
#include <cmath>
#include <vector>
...
bool chastov_v_algorithm_cannon_mpi::TestTaskMPI::RunImpl() {
  ...
  // Broadcast matrix size and total elements
  ...
  while (block_size > 0 && matrix_size_ % block_size != 0) {
    --block_size;
  }
  ...
  if (group_color == MPI_UNDEFINED) {
    return true;  // Processes not participating in computation
  }
  ...
  boost::mpi::gather(sub_world, local_c.data(), local_c.size(), collected_vec.data(), 0);
